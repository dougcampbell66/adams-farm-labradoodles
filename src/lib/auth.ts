...
//
// Known trade-offs of going stateless, both acceptable for a page with a
// handful of users:
//   • No per-session revocation. Rotating MAGIC_LINK_SECRET invalidates every
//     outstanding link and cookie at once.
//   • A magic link works more than once inside its 15-minute window. Making it
//     single-use would require storing spent nonces somewhere.
...
// Allocates over an explicit ArrayBuffer so the result is `Uint8Array<ArrayBuffer>`
// — plain `new Uint8Array(n)` widens to ArrayBufferLike, which isn't a BufferSource.
...
export function allowlist(): string[] {
  return (process.env.FOREVER_FAMILIES_ALLOWED_EMAILS ?? "")
    .split(",")
 ...
 *
 * Note this deliberately does NOT require RESEND_API_KEY. A missing mail
 * provider breaks *delivery*, not the gate, and conflating the two is what
 * turned the previous version of this system into a 503 nobody could diagnose.
 */
...
export function configProblem(): string | null {
  ...
  if (allowlist().length === 0) return "FOREVER_FAMILIES_ALLOWED_EMAILS is empty";
  return null;
}
...
async function verify(purpose: Purpose, token: string | undefined): Promise<string | null> {
  ...
  // FOREVER_FAMILIES_ALLOWED_EMAILS locks them out immediately rather than
  // whenever their 7-day cookie happens to lapse.
