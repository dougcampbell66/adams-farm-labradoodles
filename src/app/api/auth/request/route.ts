...
//  2. It must not become a way to flood an inbox. Hence the rate limit — which
//     is applied BEFORE the allowlist check and keyed on whatever address was
//     submitted, so being rate-limited says nothing about being allowlisted.
//     Reversing that order would leak the list through the difference between a
//     429 and a 303.
...
export async function POST(request: NextRequest) {
  ...
  // Local convenience: with no email provider wired up, print the link to the
  // terminal instead of silently doing nothing. Guarded on NODE_ENV so a real
  // sign-in token can never end up in production logs.
  ...
  if (!result.ok) {
    // The browser is told nothing (see the enumeration note above), so this log
    // line is the only evidence the link never went out. Make it findable and
    // make it say what to do — a bare failure here is what left this page
    // apparently-working but unreachable last time around.
    console.error(
